...
func getDesiredClusterState(observed *ObservedClusterState) *model.DesiredClusterState {
	...
	if !shouldCleanup(cluster, "PodDisruptionBudget") {
		// PodDisruptionBuget policy/v1 available in kubernetes version >= v1.21, replace policy/v1beta1
		...
		if k8sServerVersion.GreaterThanOrEqual(podDisruptionBugetV1AvailableVersion) {
			state.PodDisruptionBudget = newPodDisruptionBudgetV1(cluster)
		} else {
			state.PodDisruptionBudget = newPodDisruptionBudgetV1beta1(cluster)
		}
	}
