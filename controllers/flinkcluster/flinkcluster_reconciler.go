...
func (reconciler *ClusterReconciler) reconcilePodDisruptionBudget() error {
	...
	if desiredPodDisruptionBudget != nil && observedPodDisruptionBudget != nil {
		switch od := desiredPodDisruptionBudget.(type) {
		case *policyv1beta1.PodDisruptionBudget:
			switch oo := observedPodDisruptionBudget.(type) {
			case *policyv1.PodDisruptionBudget:
				log.Info("PodDisruptionBudget apiVersion dismatch, remove old resource, create new resource")
				err = reconciler.deletePodDisruptionBudget(oo, "PodDisruptionBudget")
				if err != nil {
					return err
				}
				err = reconciler.createPodDisruptionBudget(od, "PodDisruptionBudget")
			}
		case *policyv1.PodDisruptionBudget:
			switch oo := observedPodDisruptionBudget.(type) {
			case *policyv1beta1.PodDisruptionBudget:
	...
	if desiredPodDisruptionBudget != nil && observedPodDisruptionBudget == nil {
		err = reconciler.createPodDisruptionBudget(desiredPodDisruptionBudget, "PodDisruptionBudget")
	}
	if desiredPodDisruptionBudget == nil && observedPodDisruptionBudget != nil {
...
func (reconciler *ClusterReconciler) createPodDisruptionBudget(
	pdb runtime.Object, component string) error {
	var context = reconciler.context
	var log = reconciler.log.WithValues("component", component)
	var k8sClient = reconciler.k8sClient
	var err error
	...
	switch o := pdb.(type) {
	case *policyv1beta1.PodDisruptionBudget:
		log.Info("Creating PodDisruptionBudget", "PodDisruptionBudget", o)
		err = k8sClient.Create(context, o)
		if err != nil {
	...
	default:
		log.Error(err, "Bad PodDisruptionBudget type")
	}
...
func (reconciler *ClusterReconciler) deletePodDisruptionBudget(
	...
	switch o := pdb.(type) {
	case *policyv1beta1.PodDisruptionBudget:
		log.Info("Deleting PodDisruptionBudget", "PodDisruptionBudget", o)
		err = k8sClient.Delete(context, o)
		err = client.IgnoreNotFound(err)
		if err != nil {
